...
func backup(destination string) error {
	resp, err := http.Get("http://users-db-srv.default.svc.cluster.local:8080/api/users")
	if err != nil {
		return err
	}
	...
	if err := SendSlackNotification(destination, string(data)); err != nil {
		return err
	}
...
// r.Log.Info("Doing backup.", "destination", bac.Spec.Destination)
